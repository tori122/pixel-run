...
**완료 조건**:
- [ ] 페이지 로드 시 "Run Dude Run!" 타이틀이 표시된다
...
**완료 조건**:
- [ ] Dino가 placeholder 박스로 렌더링된다 (이미지 없는 상태)
...
**완료 조건**:
- [ ] 충돌 시 캐릭터 표정이 변한다 (dead 스프라이트/placeholder)
- [ ] 캐릭터+장애물이 함께 위로 떠오른다
- [ ] 아래로 떨어지며 사라진다
- [ ] 시퀀스 완료 후 콜백이 호출된다
- [ ] **게임 플레이가 정상 동작한다**
...
**완료 조건**:
- [ ] 충돌 시 사망 애니메이션이 재생된다
- [ ] 애니메이션 완료 후 "Game Over" + ↻ 아이콘이 표시된다
...
**완료 조건**:
- [ ] 트리거 점수 도달 시 장애물 등장이 중단된다
- [ ] 여친 쥐가 화면 오른쪽에 등장한다 (placeholder 포함)
- [ ] 주인공이 오른쪽으로 이동하여 여친 쥐에 도달한다
...
**완료 조건**:
- [ ] `GET /settings`로 게임 설정을 조회할 수 있다
